...
use redis::{Client, cmd, Connection, ConnectionAddr, ConnectionInfo, pipe, RedisResult};
...
impl Db {
    pub fn new(db_host: String, db_port: u16, db_password: Option<String>) -> Db {
        ...
        loop {
            match client.get_connection() {
                ...
                Ok(connection) => return Db { connection: connection },
            }
        }
    }
    ...
    pub fn set(&self, record: Record) -> RedisResult<()> {
        ...
        let _: () = try!(cmd("EXPIRE").arg(key.clone())
    ...
    pub fn flush(&self) -> RedisResult<()> {
        let _: () = try!(cmd("FLUSHDB").query(&self.connection));
...
fn test_db() {
    ...
    match db.get("127.0.0.1".to_owned()) {
        Ok(records) => {
            ...
            assert!(records[0].message == "<another_message>" || records[0].message == "<message>");
            assert!(records[1].message == "<another_message>" || records[1].message == "<message>");
